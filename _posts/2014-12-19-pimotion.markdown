---
layout: post
title:  "Motion detection with the Raspberry PI"
description: "A motion detection application that runs on the Raspberry PI."
date:   2014-12-19 12:52:00
categories: code 
author: jhenkens
---

Motion detection is a tricky problem to solve, the key is to balance the sensitivity so that it doesn't catch light changes but only catches movement in the frame. In this article I will describe 2 methods to achieve motion detection, calculating frame differences manually and harnessing the power of the H.264 encoder.
...
We will be using Python as the programming language. It is a nice clean language, a large collection of community developed libraries and great support. Fun fact, Eben Upton and his team imagined to use Python as the introduction to programming and it is the reason why Pi is called the Pi.

Version 1: Manual snapshots
---------------------------

Manual motion calculation requires us to take continuous photos. We use 2 buffers, one contains the current raw data and the other one contains the previous frame. Each frame we compare the 2 buffers against each other. For performance reasons, the snapshots taken every second are small thumbnail sized photos.

We loop through all the pixels in the green channel of the 2 buffers. Each pixel value from frame 1 is compared to the matching pixel in frame 2. If the value differs more than the threshold the pixel is considered different and added to the count. Once this process is completed and the total number of changed pixels is higher than the sensitivity we consider movement to be detected and take a full snapshot.
...
The idea is simple and straightforward but there is one big disadvantage to this approach. Calculating movement by comparing raw pixel values makes it overly sensitive to static, brightness changes and minor movements. These are not the movements that we like to detect. This results in a lot of false positives.

Version 2: Using motion vectors
-------------------------------

The first thing I wanted to do is to prevent using shell commands, after digging I found the [picamera python library](http://picamera.readthedocs.org/en/release-1.8/) that achieves this requirement.

The Raspberry PI's camera is capable of outputting the motion vector estimated that the H.264 encoder calculates while compressing video. We will harness this data by outputting it to a separate video channel stream.
...
With AT&T's awesome M2X python library, uploading our final montage photo to our device was possibly with just 3 lines of code. 



The full source code with installation instructions can be found on [https://github.com/citrusbyte/pimotion](https://github.com/citrusbyte/pimotion).
